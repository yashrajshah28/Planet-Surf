...
Class Master extends DBConnection {
	...
	function book_tour(){
		extract($_POST);
		// Sanitize input data
		$user_id = $this->conn->real_escape_string($this->settings->userdata('id'));
		$schedule = $this->conn->real_escape_string($schedule);
		$full_name = $this->conn->real_escape_string($full_name);
		$gender = $this->conn->real_escape_string($gender);
		$adult = intval($adult);
		$children = intval($children);
		$phone_number = $this->conn->real_escape_string($phone_number); // Sanitize phone number
		$email = $this->conn->real_escape_string($email); // Sanitize email
	
		// Prepare SQL query
		$sql = "INSERT INTO book_list (id, user_id, schedule, full_name, gender, phone_number, email, adult, children) 
				VALUES ('$user_id', '$schedule', '$full_name', '$gender', '$phone_number', '$email', $adult, $children)";
	...
	// 	// Prepare SQL query
	// 	$sql = "INSERT INTO book_list (user_id, schedule, full_name, gender, adult, children) 
	// 			VALUES ('$user_id', '$schedule', '$full_name', '$gender', $adult, $children)";
